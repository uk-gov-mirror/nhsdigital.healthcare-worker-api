...
class PractitionerRole(BaseHandler[FhirPractitionerRole]):
    def get(self, event: APIGatewayProxyEvent) -> HandlerResponse[FhirPractitionerRole]:
        ...
        if "practitioner.identifier" in event.query_string_parameters:
            ...
            _practitioner, _org_persons, roles = get_connection().search_active_nhs_person(worker_id)

            fhir_roles = [FhirPractitionerRole(pr) for pr in roles]
